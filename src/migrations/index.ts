...
export default class Migrations {
    ...
    try {
      await Promise.all(
        Object.entries(toMigrate).map(async ([k, v]) => {
          ...
          Log.log(
            'Migration',
            `${k} finished. ${result?.upsertedCount ? `Changed ${result.upsertedCount} entries` : ''} `,
          );
          succeeded.push(k);
        }),
      );
  ...
  private async saveChanges(changes: string[]): Promise<void> {
    ...
    const newElement = new Model({ changes, dbName: 'Maps' });
    await newElement.save();
  }

  private async getLastMigration(): Promise<string[]> {
    const Model = getModel(this.migrationClient as Connection);
    const entry = await Model.find({ dbName: 'Maps' });
